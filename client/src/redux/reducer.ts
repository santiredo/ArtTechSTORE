...
export default function rootReducer(state = initialState, action: Action<any>) {
  switch (action.type) {
    case "ALL_PRODUCTS": {
      return {
        ...
        allPosts: action.payload
          .filter((post: ArtGalleryItem) => post.posted === true)
          .reverse(),
        artGallery: action.payload
          .filter((post: ArtGalleryItem) => post.posted === true)
          .reverse(),
        typeFilter: action.payload
          .filter((post: ArtGalleryItem) => post.posted === true)
          .reverse(),
        techniqueFilter: action.payload
          .filter((post: ArtGalleryItem) => post.posted === true)
          .reverse(),
        allPostFilter: action.payload
          .filter((post: ArtGalleryItem) => post.posted === true)
          .reverse(),
        loadingHome: false,
      };
    ...
    case "ALL_PRODUCTS_SELLED": {
      return {
        ...state,
        allPostsSelled: action.payload
          .filter((post: ArtGalleryItem) => post.posted === false)
          .reverse(),
      };
    }
    case "RESET":
    ...
    case "DELETE_ARTIST": {
      const updatedArtists = state.artists.filter(
    ...
    case "FILTER_TYPE": {
      ...
      if (updatedTypeFilters.includes(action.payload)) {
        updatedTypeFilters = updatedTypeFilters.filter(
          (filter) => filter !== action.payload
        );
      } else {
        updatedTypeFilters.push(action.payload);
      }
      const filteredByType = state.allPosts.filter((post) =>
        updatedTypeFilters.includes(post.type)
      );
    ...
    case "FILTER_TECHNIQUE": {
      ...
      if (updatedTechniqueFilters.includes(action.payload)) {
        updatedTechniqueFilters = updatedTechniqueFilters.filter(
          (filter) => filter !== action.payload
        );
      } else {
        updatedTechniqueFilters.push(action.payload);
      }
      const filteredByTechnique = state.allPosts.filter((post) =>
        updatedTechniqueFilters.includes(post.technique)
      );
