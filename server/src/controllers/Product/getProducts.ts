...
export async function getProducts() {
  ...
  let artistIds: number[] = allProducts.map(
    (product) => product.dataValues.ArtistId
  );

  let artistPromises: Promise<ArtistInterface | null>[] = artistIds.map(
    async (id) => {
